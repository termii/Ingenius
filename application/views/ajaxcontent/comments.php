...
<br />
<h4><?php if (count($stories) > 0) echo count($stories);  ?> Comment<?php if (count($stories) > 1) { ?>s <?php } ?></h4>
<?php
...
<?php foreach($stories as $row): ?>
	...
	<li class="box row">
		...
		<?php if ($row['users_id'] == $this->session->userdata('userid')) { ?>
                        <div class="removebtn" style="position:absolute;top:0px;right:0px;z-index:9999;">
                          <a href="#" style="cursor:hand;" class="btn-danger btn-sm rembtn" data-id="<?php echo $row['comment_id']; ?>">DELETE</a>
                        </div>
                        <?php } ?>
    ...
    <br style="clear:both;" />
    <div class="replybtn"><a style="font-size:11px;" href="javascript:void(0);" onclick="jQuery('#commentform-<?php echo $row['comment_id']; ?>').toggle();">Reply</a></div>
	...
	<li class="box row" style="padding-left:50px;">
        ...
        <div class="removebtn">
            <a href="#" style="cursor:hand;" class="btn-danger btn-sm remreplybtn" data-id="<?php echo $rep['comment_id']; ?>">DELETE</a>
        </div>
   		<?php } ?>
	...
	<div style="display:none;" id="commentform-<?php echo $row['comment_id']; ?>" class="row">
		...
		<div class="col-md-6">
			<textarea name="message" placeholder="Reply" rows="1"  class="replybox" style="margin-top:0;"></textarea>
		</div>
		<div class="col-md-4" style="text-align:left;">
			<input type="submit" value="Send" style="margin-top:0;" class="button-green" /></a>
		</div>
		</form>
	</div>
...
<p>&nbsp;</p>
<?php else: ?>	
	<span style="color:#888;">No comments yet.</span>
	<br /><br />
<?php endif; ?>
